...
struct TensorCI2Param {
    ...
    vector<vector<Real>> weight;                            ///< TODO: weight for the tt sum. When not empty, it activates the ENV learning
    ...
    operator ARRLUParam<T>() const { return {.reltol = reltol, .bondDim = bondDim, .fullPiv = fullPiv, .nRookIter = nRookIter}; }
};
...
struct TensorCI2 {
    ...
    TensorCI2Param<T> param;                                ///< parameters of the algorithm
    vector<Real> pivotError;                                ///< max pivot error for each rank
    ...
    vector<Mat> P;                                          ///< the pivot matrix in LU form for each bond
    ...
    TensorCI2(TensorFunction<T> const& f_, vector<int> localDim, TensorCI2Param<T> param_ = {})
        : f{f_},
        param(param_),
        pivotError(1, Real(0)),
        Iset(localDim.size()),
        ...
        J0(localDim.size())
    {
        ...
        pivotError[0] = abs(f.f(param.pivot1));
        ...
        for (auto p = 0u; p < len(); p++)
            for (int i = 0; i < localDim[p]; i++)
                localSet[p].push_back({char32_t(i)});

        //add param.pivot1;
        for (auto b = 0u; b < len(); b++) {
            Iset[b].push_back({param.pivot1.begin(), param.pivot1.begin() + static_cast<int>(b)});
            Jset[b].push_back({param.pivot1.begin() + static_cast<int>(b) + 1, param.pivot1.end()});
        }
        if (param.cond) {
            ...
            fCond = TensorFunction<unsigned int>{[this](vector<int> const& id) -> unsigned int { return param.cond(id) ? 1 : 0; }};
        }
        iterate(1, 0); // just to define tt
    }

    /// constructs a rank-1 TensorCI2 from a function f:(a1,a2,...,an)->eT  where the index ai is in [0,localDim[i]).
    TensorCI2(function<T(vector<int>)> f_, vector<int> localDim, TensorCI2Param<T> param_ = {})
    ...
    TensorCI2(function<T(vector<int>)> f_, TensorTrain<T> tt_, TensorCI2Param<T> param_ = {})
        : f{f_, param_.useCachedFunction}, param(param_), pivotError(1, Real(0))
        , Iset(tt_.M.size()), localSet(tt_.M.size()), Jset(tt_.M.size())
        , tt(tt_.M.size()), P(tt_.M.size())
        , pivotErrorAll(tt_.M.size()-1), I0(tt_.M.size()), J0(tt_.M.size())
    {
        // fill localSet, Iset Jset
        for (auto p = 0u; p < len(); p++)
            for (Eigen::Index i = 0; i < tt_.M[p].n_cols; i++)
                localSet[p].push_back({char32_t(i)});
        Iset[0].push_back({});
        ...
        for (auto b = 0u; b+1 < tt_.M.size(); b++) {
            ...
            auto ci = CURDecomp<T>(cube_as_matrix2(tt_.M[b]), true, Real(0), param.bondDim);
            Mat M1 = ci.left();
            Mat M2 = ci.right() * cube_as_matrix1(tt_.M[b+1]);
            tt_.M[b] = Cube(M1.data(), tt_.M[b].n_rows, tt_.M[b].n_cols, M1.cols());
            tt_.M[b+1] = Cube(M2.data(), M2.rows(), tt_.M[b+1].n_cols, tt_.M[b+1].n_slices);
            Iset[b+1] = Ib.at(ci.row_pivots());
        }
        // right-to-left
        for (int b = static_cast<int>(tt_.M.size())-2; b >= 0; b--) {
            IndexSet<MultiIndex> Jb = kron(localSet[b+1], Jset[b+1]);
            auto ci = CURDecomp<T>(cube_as_matrix1(tt_.M[b+1]), false, Real(0), param.bondDim);
            Mat M1 = cube_as_matrix2(tt_.M[b]) * ci.left();
            Mat M2 = ci.right();
            tt_.M[b] = Cube(M1.data(), tt_.M[b].n_rows, tt_.M[b].n_cols, M1.cols());
            tt_.M[b+1] = Cube(M2.data(), M2.rows(), tt_.M[b+1].n_cols, tt_.M[b+1].n_slices);
        ...
        if (param.cond) {
            if (!param.cond(param.pivot1))
                throw std::invalid_argument("First pivot does not fulfill the condition function");
            fCond = TensorFunction<unsigned int>{[this](vector<int> const& id) -> unsigned int { return param.cond(id) ? 1 : 0; }};
        }
    ...
    TensorCI2(TensorTrain<T> const& tt_, TensorCI2Param<T> param_ = {}) : TensorCI2(tt_, tt_, param_) {}
    ...
    void addPivots(Tci const& ci)
    {
        ...
        for (auto b = 0u; b < ci.len()-1; b++)
            addPivotsAt(ci.getPivotsAt(b), static_cast<int>(b));
        iterate(1, 0);
    }
    ...
    void iterate(int nIter = 1, int dmrg_type = 2)
    {
        for (auto i = 0; i < nIter; i++) {
            if (cIter%2 == 0)
                for (auto b = 0u; b < len()-1; b++) { center = static_cast<int>(b)+1; updatePivotAt(static_cast<int>(b), dmrg_type); }
            else
                for (int b = static_cast<int>(len())-2; b >= 0; b--) { center = b; updatePivotAt(b, dmrg_type); }
            cIter++;
        }
    }
    ...
    Real trueError(size_t max_n_eval = 1e6) const { return tt.trueError(f.f, max_n_eval); }
    ...
    bool isDone() const
    {
        Real ref = *std::max_element(pivotError.begin(), pivotError.end()) * Real(param.reltol);
        for (auto b = 0u; b < len()-1; b++) {
            int rankMax = std::min(static_cast<int>(Iset[b].size()*localSet[b].size()),
                                   static_cast<int>(localSet[b+1].size()*Jset[b+1].size()));
            rankMax = std::min(rankMax, param.bondDim);
            if (static_cast<int>(Iset[b+1].size()) < rankMax &&         // not saturated
...
protected:
    ...
    void dmrg1_updatePivotAt(int b)
    {
        ...
        IndexSet<MultiIndex> Ib = isLeft ? IndexSet<MultiIndex>(kron(Iset[b], localSet[b])) : IndexSet<MultiIndex>(Iset[b+1].from_int());
        IndexSet<MultiIndex> Jb = isLeft ? IndexSet<MultiIndex>(Jset[b].from_int()) : IndexSet<MultiIndex>(kron(localSet[b+1], Jset[b+1]));
        ...
        Jset[b] = Jb.at(ci.col_pivots());
        P[b] = ci.PivotMatrixTri();
        if (isLeft) { set_site_tensor(b, ci.left()); set_site_tensor(b+1); }
        else { set_site_tensor(b); set_site_tensor(b+1, ci.right()); }
        collectPivotError(b, ci.pivotErrors());
    }
    ...
    void set_site_tensor(int b, Mat const& M) {
        tt.M[b] = Cube(M.data(), Iset[b].size(), localSet[b].size(), Jset[b].size());
    }

    void collectPivotError(int b, vector<Real> const& pe)
    {
        pivotErrorAll[b] = pe;
        if (pe.size() > pivotError.size()) pivotError.resize(pe.size(), Real(0));
        for (auto i = 0u; i < pe.size(); i++)
            if (pe[i] > pivotError[i]) pivotError[i] = pe[i];
    }

    vector<vector<Real>> pivotErrorAll;                 ///< The pivot error list for each bonds
...
public:
    ...
    using typename TensorCI2<T>::Mat;
    using TensorCI2<T>::P;
    using TensorCI2<T>::center;

    /// constructs a rank-1 CTensorCI2 from a function f:(u1,u2,...,un)->T  where ui is in xi[i].
    CTensorCI2(function<T(vector<Index>)> f_, vector<vector<Index>> const& xi_, TensorCI2Param<T> par = {})
    ...
    {
        CTensorTrain<T, Index> ctt;
        for (int p = 0; p < center; p++)
            ctt.M.push_back([this, p](Index x) { return get_TP1_at(p, x); });
        ctt.M.push_back([this](Index x) { return get_T_at(center, x); });
        for (int p = center+1; p < static_cast<int>(this->len()); p++)
            ctt.M.push_back([this, p](Index x) { return get_P1T_at(p, x); });
        return ctt;
    }
    ...
    Mat get_TP1_at(int p, Index x) const { return compute_CU_on_rows(get_T_at(p, x), P.at(p)); }
    
    /// return the value of get_T_at(p,x)*P[p]^-1 where P[p] is the pivot matrix at bond p.
    Mat get_P1T_at(int p, Index x) const { return compute_UR_on_cols(get_T_at(p, x), P.at(p-1)); }
