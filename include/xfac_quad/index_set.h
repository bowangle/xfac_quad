...
//  WARNING MultiIndexHash is defined here!
...
inline vector<int> set_diff(int n, vector<int> Iset)
{
    ...
    std::set_difference(all.begin(),all.end(),
                        Iset.begin(),Iset.end(),std::back_inserter(diff));
    return diff;
}

inline vector<int> take_n_random(vector<int> A, size_t n)
{
    vector<int> out;
...
public:
    ...
    {
        if (to_int_data.find(i) != to_int_data.end()) return;
...
// Hash for MultiIndex (std::u32string)
struct MultiIndexHash {
    size_t operator()(MultiIndex const& mi) const noexcept {
        size_t seed = mi.size();
        for (char32_t x : mi) {
