...
constexpr Real ci_tol2   = Real(100000) * std::numeric_limits<Real>::epsilon();
...
TEST_CASE("Test MatrixCI1 - double", "[matrix_ci1]")
{
    SECTION("fixed rank")
    {
        int n = 100, rank = 50;
        ...
        REQUIRE(check_lt("tt.eval", std::abs(ci.get_TensorTrain().eval({i, j}) - A(i, j)), ci_tol2 * A.norm()));
    }
...
TEST_CASE("Test MatrixCI1 complex - double", "[matrix_ci1]")
{
    SECTION("rank-2 separable function")
    {
        auto myf = [](vector<int> const& id) -> CT {
            return CT(1, 2) * CT(id[0] + 1.0, 0) * CT(id[1] + 2.0, 0)
                 + CT(3, -1) * CT(id[0] * 4.0, 0) * CT(id[1] + 5.0, 0);
        };
        TensorCI1Param<CT> p;
        p.nIter = 10;
        p.fullPiv = true;
    ...
    SECTION("continuous function - directly")
    {
        ...
        REQUIRE(check_lt("ctt.eval", std::abs(ci.get_CTensorTrain().eval({x, y}) - myTf({x, y})), ci_tol2));
    }
}
...
TEST_CASE("Test MatrixCI2 - double", "[matrix_ci2]")
{
    SECTION("fixed rank")
    {
        int n = 100, rank = 50;
        ...
        REQUIRE(check_le("trueError", ci.trueError(), ci_tol2 * A.norm()));
        int i = rand() % A.rows(), j = rand() % A.cols();
        REQUIRE(check_lt("tt.eval", std::abs(ci.tt.eval({i, j}) - A(i, j)), ci_tol2 * A.norm()));
    }
    ...
    SECTION("continuous function - directly")
    {
        ...
        REQUIRE(check_lt("ctt.eval", std::abs(ci.get_CTensorTrain().eval({x, y}) - myTf({x, y})), ci_tol2));
        ci.makeCanonical();
        REQUIRE(check_lt("ctt.eval canonical", std::abs(ci.get_CTensorTrain().eval({x, y}) - myTf({x, y})), ci_tol2));
    }
}
...
TEST_CASE("Test CrossData - double", "[cross]")
{
    ...
    while (static_cast<int>(I.size()) < maxPivots) {
        ...
        // build AIJ, AiJ, AIj using Eigen indexing
        ...
        Eigen::RowVectorXd AiJ(static_cast<Eigen::Index>(J.size()));
        for (size_t rj = 0; rj < J.size(); rj++)
            AiJ(static_cast<Eigen::Index>(rj)) = A(i, J[rj]);

        Eigen::VectorXd AIj(static_cast<Eigen::Index>(I.size()));
        for (size_t ri = 0; ri < I.size(); ri++)
            AIj(static_cast<Eigen::Index>(ri)) = A(I[ri], j);

        Real err = std::abs(A(i,j) - static_cast<Real>((AiJ * AIJ.inverse() * AIj)(0)));
    ...
    SECTION("interpolation") {
        ...
        SECTION("each row/col") {
            for (int i : I)
                REQUIRE(std::abs(cross.eval(i, 0) - A(i, 0)) < ci_tol * 100);  // check one column
        }
    }
    SECTION("insert") {
...
TEST_CASE("Test RRLUDecomp - double", "[rrlu]")
{
    SECTION("2x3 matrix") {
        ...
        SECTION("vertical matrix cond") {
            Mat At = A.transpose();
            MatUint condT = MatUint(2, 3);
            condT << 0, 1, 1, 1, 1, 0;
            MatUint condT_t = condT.transpose();
            for (auto isLeft : {0, 1}) {
...
TEST_CASE("Test ARRLUDecomp - double", "[arrlu]")
{
